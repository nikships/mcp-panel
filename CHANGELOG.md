...
### Fixed
- **Update Check Crash** - Fixed a hard crash that occurred when "Check for Updates" (or the automatic background check) found a newer version. The DMG workflow embedded a hardcoded Sparkle **2.5.2** framework while the app binary was compiled against the version pinned in `Package.resolved` (2.9.2). Showing the "update available" window loaded a nib whose Cocoa Bindings didn't match the linked binary, throwing an uncaught KVO exception. The workflow now embeds the Sparkle framework (and `generate_appcast` tool) version read from `Package.resolved`, keeping the embedded framework and the linked binary in sync.
